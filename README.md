...
- **成员/分组**：本地清单 `content/coaching.ts`（zod 校验）—— 唯一决定「哪些测评属于 coaching、如何分 niche」。
- 本站**不渲染结果页**（交给 cairo）、**不改动 Payload 后端**。

## 目录

```
content/coaching.ts        # 本地清单（分类 + 测评 publicToken + SEO 覆盖）
src/lib/env.ts             # 环境变量（带生产默认值）
...
2. 在 `content/coaching.ts` 的 `quizzes` 里加一条（指定 `slug`、`categorySlugs`，可选 `seo`）。
